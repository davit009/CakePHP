...
class ReactivosController extends AppController
{
    public function index()
    {
        ...
        $query = ($user->role === 'usuariobase')
            ? $this->Reactivos->find()->where(['user_id' => $user->id])
            : $this->Reactivos->find()->contain(['Users']);
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            $data = $this->request->getData() + ['user_id' => $this->Authentication->getIdentity()->id];
            $reactivo = $this->Reactivos->patchEntity($reactivo, $data);
            if ($this->Reactivos->save($reactivo)) {
                $this->Flash->success('El reactivo ha sido guardado.');
                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error('El reactivo no pudo ser guardado. Por favor, intenta nuevamente.');
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Reactivos->delete($reactivo)) {
            $this->Flash->success('El reactivo ha sido eliminado.');
        } else {
            $this->Flash->error('El reactivo no pudo ser eliminado. Por favor, intenta nuevamente.');
        }
        return $this->redirect(['action' => 'index']);
    }
}
