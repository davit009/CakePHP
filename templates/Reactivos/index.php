...
<div class="reactivos index content">
 <h3>
 <?= $user->role === 'administrador' ? 'Todos los Reactivos' : 'Mis
Reactivos' ?>
 </h3>

 <div class="table-actions">
 <?= $this->Html->link(__('Crear Nuevo Reactivo'), ['action' => 'add'],
['class' => 'button float-right']) ?>
 ...
 <th><?= $this->Paginator->sort('subespecialidad',
'Subespecialidad') ?></th>
 <th><?= $this->Paginator->sort('dificultad', 'Dificultad') ?></th>
 <th><?= $this->Paginator->sort('respuesta_correcta', 'Resp.
Correcta') ?></th>
 <?php if ($user->role === 'administrador'): ?>
 <th><?= $this->Paginator->sort('Users.email', 'Creado por')
?></th>
 <?php endif; ?>
 <th><?= $this->Paginator->sort('created', 'Fecha Creación')
?></th>
...
?>">
 ...
 <td class="actions">
 <?= $this->Html->link(__('Ver'), ['action' => 'view', $reactivo->id],
['class' => 'button button-outline button-small']) ?>
 <?php if ($user->role === 'administrador' || $reactivo->user_id
== $user->id): ?>
<?= $this->Html->link(__('Editar'), ['action' => 'edit', $reactivo->id], ['class' => 'button button-clear button-small']) ?>
 <?= $this->Form->postLink(__('Eliminar'), ['action' => 'delete',
$reactivo->id], ['confirm' => __('¿Estás seguro de que quieres eliminar este
reactivo?'), 'class' => 'button button-outline button-small delete-btn']) ?>
 <?php endif; ?>
 </td>
 </tr>
 <?php endforeach; ?>
 ...
 <?= $this->Html->link('Crear el primer reactivo', ['action' => 'add'], ['class'
=> 'button']) ?>
 </div>
 <?php endif; ?>
...
mostrando {{current}} registro(s) de {{count}} total')) ?></p>
 </div>
</div>
<style>
.table-actions {
