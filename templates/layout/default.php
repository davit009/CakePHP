...
<head>
    ...
    <title>Sistema de Exámenes: <?= $this->fetch('title') ?></title>
    <?= $this->Html->meta('icon') ?>
    <?= $this->Html->css(['normalize.min','milligram.min','fonts','cake','home']) ?>
    <?= $this->fetch('meta') ?>
    <?= $this->fetch('css') ?>
    <style>
    .navbar{background:#2c3e50;padding:1rem;margin-bottom:2rem}
    .navbar a{color:#fff;text-decoration:none;margin-right:1rem}
    .navbar a:hover{color:#3498db}
    .user-info{float:right;color:#fff}
    .dashboard-card{border:1px solid #ddd;padding:1.5rem;margin:1rem 0;border-radius:5px;background:#f8f9fa}
